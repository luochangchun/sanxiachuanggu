<template>
    ...
    <div class="show_need">
        ...
        <div class="need_import">
            <h3>{{data['title']}}</h3>
            <p>{{data['enterprise']}} | 需求类型: <span v-for="n in searchConsult" :key="n" v-if="n['id'] == data['businessId']">{{n['value']}}</span> | 联系人: {{data['contact']}} | 联系方式: {{data['phone']}} | 2017-10-24 | <span v-if="data['status'] == 1">待解决</span> <span v-if="data['status'] == 2">进行中</span><span v-if="data['status'] == 3">已解决</span></p>
